...
import { getProblems, type Problem, type ProblemType } from "~/lib/server-actions/problems";
...
export async function loader({}: Route.LoaderArgs): Promise<{problems: Problem[], courses: any[]}> {
    try {
        ...
        const problems = problemsResponse.ok ? await problemsResponse.json() as Problem[] : [];
        const courses = coursesResponse.ok ? await coursesResponse.json() as any[] : [{ id: "default", name: "Default Course" }];
...
export function meta({}: Route.MetaArgs): Route.MetaDescriptors {
    return [
        ...
        { name: "description", content: `Browse programming problems and challenges on ${Config.sitename}. Filter by course, type, and difficulty.` },
    ];
}

export default function Problems() {
    const { problems: initialProblems, courses } = useLoaderData<{problems: Problem[], courses: any[]}>();
    ...
    useEffect(() => {
        ...
        if (selectedCourseId && selectedCourseId !== "all") {
            filtered = filtered.filter((problem) => 
                problem.course.courseId === selectedCourseId
            );
        }
        ...
        setCurrentPage(1);
    }, [searchTerm, problems, sortField, sortOrder, selectedCourseId, selectedProblemTypes, sortProblems]);
    ...
    return (
        <main className="max-w-7xl mx-auto py-8 px-4">
            {!isLoaded && <Loading />}
            <div className={`problems-page-container ${isLoaded ? "loaded" : "loading"}`}>
                <div className="mb-6">
                    ...
                    <div className="space-y-4 mb-6">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center gap-2">
                                <FontAwesomeIcon icon={faFilter} className="text-sm text-muted-foreground" />
                            ...
                            <div className="flex flex-wrap gap-2">
                                {PROBLEM_TYPES.map((type) => (
                                    <Button
                                        key={type}
                                        variant={selectedProblemTypes.includes(type) ? "default" : "outline"}
                                        size="sm"
                        ...
                        <table className="data-table">
                            <thead>
                                <tr className="data-table-header">
                                    ...
                                    <th className="data-table-header-cell" style={{ width: "12rem" }}>
                                        Types
                                    </th>
                                    <th
                                        className="data-table-header-cell sortable"
                                                ...
                                                <div className="flex flex-wrap gap-1">
                                                    {problem.type.map((type) => (
                                                        <Badge
                                                            key={type}
                                                            variant="secondary"
                                                            className={`text-white ${getBadgeColor(type)} text-xs`}
                                                        >
                                                            {type}
                                                        </Badge>
                                                    ))}
                ...
                {currentProblems.length > 0 && (
                    <div className="mt-6 text-sm text-muted-foreground">
                        <p>
                            Click on a problem name to view details and start solving.
                        </p>
                    </div>
                )}
            </div>
